...
output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v6_casp/job_{JOB_CHUNK_NUMBER}/"
plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v6_casp/job_{JOB_CHUNK_NUMBER}/"
...
for img_path in tqdm(image_paths, desc=f"Job {JOB_CHUNK_NUMBER} Processing Images"):
    ...
    with torch.no_grad():
        ...
        #     # Get keypoints from resized image
        #     keypoints, keypoints_score, data_samples = process_frame_np(resized_img_rgb)

        #     # Scale keypoints back to original image size
        #     keypoints = ((keypoints - [pad_x, pad_y]) / scale).astype(int)

        #     # Ensure keypoints are within valid image bounds
        #     H, W = depth_map.shape
    ...
    if DEBUG_MODE and processed_frames < DEBUG_FRAMES_COUNT:
        ...
        axes[0, 1].set_ylim(y_max_padded, y_min_padded)
        ...
        radius_ratio = max(radius_values) / max(min(radius_values), 0.1)
