...
# output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v3/job_{JOB_CHUNK_NUMBER}/"
# plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v3/job_{JOB_CHUNK_NUMBER}/"

# V5: we fix the gt sampling
USE_GT_FOR_DAV = False
# output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v5_gtdav/job_{JOB_CHUNK_NUMBER}/"
# plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v5_gtdav/job_{JOB_CHUNK_NUMBER}/"


# output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v4_correct_highres/job_{JOB_CHUNK_NUMBER}/"
# plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v4_correct_highres/job_{JOB_CHUNK_NUMBER}/"


# output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v8_cache_maps/job_{JOB_CHUNK_NUMBER}/"
# plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v8_cache_maps/job_{JOB_CHUNK_NUMBER}/"


output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_h36m_v10_cache_maps/job_{JOB_CHUNK_NUMBER}/"
plots_output_dir = f"/srv/essa-lab/flash3/nwarner30/pose_estimation/coarse_depth_experiments/Depth-Anything-V2/test_outputs_metric_plots_h36m_v10_cache_maps/job_{JOB_CHUNK_NUMBER}/"
...
if USE_GT_FOR_DAV:
    train_ann_file = '/srv/essa-lab/flash3/nwarner30/pose_estimation/h36m_data/annotations/h36m_train_vanilla_50hz_h36m_50hz.npz'
    train_annotations = np.load(train_ann_file, allow_pickle=True)
    train_imgnames = train_annotations['imgname']
    train_gt_keypoints = train_annotations['part']
    val_ann_file = '/srv/essa-lab/flash3/nwarner30/pose_estimation/h36m_data/annotations/h36m_test_vanilla_50hz_h36m_50hz.npz'
    val_annotations = np.load(val_ann_file, allow_pickle=True)
    val_imgnames = val_annotations['imgname']
    val_gt_keypoints = val_annotations['part']



    combined_imgnames = np.concatenate([train_imgnames, val_imgnames])
    combined_keypoints = np.concatenate([train_gt_keypoints, val_gt_keypoints])
    # ipdb.set_trace()
...
for img_path in tqdm(image_paths, desc=f"Job {JOB_CHUNK_NUMBER} Processing Images"):
    ...
    with torch.no_grad():
        ...
        if USE_GT_FOR_DAV:
            ...
            subject = parts[1]  # '01'
            action = parts[3]   # '02'
            subaction = parts[5]  # '01'
            camera = parts[7]    # '01'
            frame = parts[8].split('.')[0]  # '000809'

            # Construct the formatted name
            formatted_name = f"{int(subject)}_{int(action)}_{int(subaction)}-0_{int(camera)}_img_{frame}.jpg"
            ...
            # raw_img, scale, pad_x, pad_y = pad_resize(raw_img, target_size=(388, 388))
            # Run detection on the raw full-res frame
    ...
    if not USE_GT_FOR_DAV and rtm_feature_coarse is not None:
        save_dict['rtm_feature_coarse'] = rtm_feature_coarse
        save_dict['rtm_bbox'] = selected_bbox  # [x1, y1, x2, y2] in full image coords
    
    # Add DAV2 256-D feature map (fp16)
    save_dict['dav2_feature_256'] = dav2_feature_256
